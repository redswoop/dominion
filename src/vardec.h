...
#if 0 /* old userrec — disabled for macOS port */
typedef struct {
	char		name[31],		/* user's name */
			realname[21],		/* user's real name */
			callsign[7],		/* user's amateur callsign */
			...
			note[41],		/* sysop's note about user */
                        ...
                        city[41],
                        state[2],
                        macros[4][MAX_PATH_LEN],          /* macro keys */
			sex;			/* user's sex */
...
typedef struct {
	char		newuserpw[21],		/* new user password */
			...
			msgsdir[MAX_PATH_LEN],		/* path for msgs directory */
			gfilesdir[MAX_PATH_LEN],		/* path for gfiles dir */
			datadir[MAX_PATH_LEN],		/* path for data directory */
			dloadsdir[MAX_PATH_LEN],		/* path for dloads dir */
			ramdrive,		/* drive for ramdisk */
			tempdir[MAX_PATH_LEN],		/* path for temporary directory */
...
typedef struct {
	char		name[41],		/* directory name */
			filename[9],		/* direct database filename */
                        dpath[MAX_PATH_LEN],               /* filename path */
                        upath[12],
                        vacs[21],
                        res[48],
                        acs[21];
...
// Post Data
typedef struct {
	char		title[MAX_PATH_LEN];		/* title of post */
...
// Email Data
typedef struct {
	char		title[MAX_PATH_LEN];		/* E-mail title */
...
// Short Message Data
typedef struct {
	char		message[MAX_PATH_LEN];		/* short message to user */
...
// Voting Responses
typedef struct {
	char		response[MAX_PATH_LEN];		/* Voting question response */
...
typedef struct {
	char		question[MAX_PATH_LEN];		/* Question */
