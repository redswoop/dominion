...
/* Legacy message base globals — were removed when JAM was added */
static postrec msgs[512];
static unsigned long sub_dates[MAX_SUBS];
static char irt[MAX_PATH_LEN], irt_name[MAX_PATH_LEN], irt_from[MAX_PATH_LEN];
static int fidotoss;

/* getorigin() removed — already defined in msgbase.c */
...
int fidotosser(int bn)
{
    ...
    char s[MAX_PATH_LEN], s1[MAX_PATH_LEN], c;
    long len;
    char *b;
    fmsgrec f;
...
int fidoscan(int bn, int *tot)
{
    ...
    char s[MAX_PATH_LEN], s1[MAX_PATH_LEN], c, i2 = 1;
    long len;
    char *b;
