...
void main(int ac, char **ar)
{
    ...
    char *b,*subfld;
    configrec syscfg;
    int numsubs,whichsub;
    subboardrec subs[200];
    ...
    read(i,&syscfg,sizeof(configrec));
    close(i);
    sprintf(s,"%ssubs.dat",syscfg.datadir);
    i=open(s,O_RDWR|O_BINARY);
    numsubs=((read(i,&subs[0],200*sizeof(subboardrec)))/sizeof(subboardrec));
    close(i);

    for(whichsub=0;whichsub<numsubs;whichsub++) {

        sprintf(basefn,"%s%s",syscfg.msgsdir,subs[whichsub].filename);

        JAMOpen(basefn,&JamRec);
        nummsgs=filelength( JamRec.IdxHandle )/sizeof(JAMIDXREC);
        printf("%d messages\n",nummsgs);

        numtokeep=subs[whichsub].maxmsgs;
        ...
        printf("Packing %s (%s) from %d to %d\n",subs[whichsub].name,subs[whichsub].filename,nummsgs,subs[whichsub].maxmsgs);
        JAMOpen("temp",&Jr);
