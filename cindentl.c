...
FILE *infile,*outfile,*list;

void indenter (char fn[81])
{
    int k;
...
void main(int argc, char *argv[])
{
    int i,i1;
    FILE *out, *tmpin;
    char *ss,s[161],s1[181];

    if (argc!=2) {
        printf("Run the Cindentl program only from the makefile.\n\n");
        exit(-1);
    }

    tmpin = fopen(argv[1],"r");
    do {
        i1=fscanf(tmpin,"%s",s);
        if (i1>0) {
