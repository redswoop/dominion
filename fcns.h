...
int lfs(char spec[12],char ss[81],int *abort,long *bytes,int new);
...
void rundoor(char type,char ms[81]);


/* File: mm1.c */

char *getfmt(char *fn, int which);
void parsemmfmt(char line[81]);
...
int ccount(char s[81]);
char *aligncmd(char in[81]);
void drawhead(int top);
void showmenu();
void plfmt(char *s);
void plfmta(char *s,int *abort);
//int mslok(char val[81],char inp[81],int qyn,varrec *vars,int numvars);
...
void gnscan(void);
void email(int u,char subject[81],int ask);
...
void reqchat1(char reason[81]);
void chatsound();
void reqchat(char reason[81]);
